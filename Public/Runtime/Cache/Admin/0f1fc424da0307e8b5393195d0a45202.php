...
<div>
	<ul id="content-public-category-tree" class="easyui-tree" data-options="url:'<?php echo U('Content/public_category');?>',animate:true,lines:true,onClick:contentCategoryModule.click"></ul>
</div>
...
var contentCategoryModule = {
	...
	formatter:function(node){
		var res = node.text;
		if (!node.children || node.children.length == 0){
			var typeList = <?php echo json_encode(dict('type', 'Category'));?>;
	...
	open: function(type, id, crumbs){
		switch(parseInt(type)){
			//页面
			case 0:
				var url = "<?php echo U('Content/page');?>";
			...
			//列表
			case 1:
				var url = "<?php echo U('Content/article');?>";
				url += url.indexOf('?') != -1 ? '&catid='+id : '?catid='+id;
				contentBaseModule.openUrl(url, false, crumbs);
				break;
